...
public:
  ...
  static String GetBC26Data()
  {
    String data;
    long begin = millis();
    while (Serial.available() && millis() - begin <= 5000)
    {
      delay(10);
      char c = Serial.read();
      if (c == '\n')
  ...
  static void Sleep(int seconds, RTClock &rt, int &timeoutCount)
  {
    ...
    delay(500);
    sleepAndWakeUp(STANDBY, &rt, seconds);
  }
