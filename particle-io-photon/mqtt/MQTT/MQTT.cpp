...
MQTT::MQTT(const char *domain, uint16_t port, void (*callback)(char *, uint8_t *, unsigned int),
           bool thread)
{
    this->initialize(NULL, domain, NULL, port, MQTT_DEFAULT_KEEPALIVE, MQTT_MAX_PACKET_SIZE, callback, thread);
}

MQTT::MQTT(const char *domain, uint16_t port, int maxpacketsize, void (*callback)(char *, uint8_t *, unsigned int),
           bool thread)
{
    this->initialize(NULL, domain, NULL, port, MQTT_DEFAULT_KEEPALIVE, maxpacketsize, callback, thread);
}

MQTT::MQTT(const uint8_t *ip, uint16_t port, void (*callback)(char *, uint8_t *, unsigned int),
           bool thread)
{
    this->initialize(NULL, NULL, ip, port, MQTT_DEFAULT_KEEPALIVE, MQTT_MAX_PACKET_SIZE, callback, thread);
}

MQTT::MQTT(const char *rootCA, const uint8_t *ip, uint16_t port, void (*callback)(char *, uint8_t *, unsigned int))
{
    this->initialize(rootCA, NULL, ip, port, MQTT_DEFAULT_KEEPALIVE, MQTT_MAX_PACKET_SIZE, callback, NULL);
}

MQTT::MQTT(const char *rootCA, const char *domain, uint16_t port, void (*callback)(char *, uint8_t *, unsigned int))
{
    this->initialize(rootCA, domain, NULL, port, MQTT_DEFAULT_KEEPALIVE, MQTT_MAX_PACKET_SIZE, callback, NULL);
}

MQTT::MQTT(const uint8_t *ip, uint16_t port, int maxpacketsize, void (*callback)(char *, uint8_t *, unsigned int),
           bool thread)
{
    this->initialize(NULL, NULL, ip, port, MQTT_DEFAULT_KEEPALIVE, maxpacketsize, callback, thread);
}

MQTT::MQTT(const char *domain, uint16_t port, int maxpacketsize, int keepalive, void (*callback)(char *, uint8_t *, unsigned int),
           bool thread)
{
    this->initialize(NULL, domain, NULL, port, keepalive, maxpacketsize, callback, thread);
}

MQTT::MQTT(const uint8_t *ip, uint16_t port, int maxpacketsize, int keepalive, void (*callback)(char *, uint8_t *, unsigned int),
           bool thread)
{
    this->initialize(NULL, NULL, ip, port, keepalive, maxpacketsize, callback, thread);
}
...
void MQTT::initialize(const char *rootCA, const char *domain, const uint8_t *ip, uint16_t port, int keepalive, int maxpacketsize,
...
bool MQTT::connect(const char *id, const char *user, const char *pass, const char *willTopic, EMQTT_QOS willQos, uint8_t willRetain, const char *willMessage, bool cleanSession, MQTT_VERSION version)
{
    if (!isConnected())
    {
        ...
        int result = 0;
        _client.init(this->rootCA, strlen(this->rootCA) + 1);

        if (this->ip == NULL)
            result = _client.connect((char *)this->domain.c_str(), this->port);
        else
...
{
    bool rc = (int)_client.isConnected();
    if (!rc)
        _client.stop();
    return rc;
}
